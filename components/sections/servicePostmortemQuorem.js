...
const Serviceone = ({ service }) => {
  ...
  const pageUrl = `${process.env.NEXT_PUBLIC_SITE_URL}/postmortem-quorem-encryption`;

  return (
    <>
    <Head>
        <title>Postmortem Quorum Encryption</title>
        <meta name="description" content={pageDescription} />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <meta name="keywords" content="Postmortem Quorum Encryption" />
                      ...
                      <ul>
                        {subServices.map((subService, index) => (
                          <li key={subService.id}>
                            <Link className={(index === 4) ? 'current': ''} href={subService.link || ''}>
